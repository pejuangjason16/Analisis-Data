...
# Melakukan mapping pada cuaca
weather_mapping = {1: "Cerah", 2: "Berawan", 3: "Hujan/Snow"}
df['weather_name'] = df['weathersit'].map(weather_mapping)

# Mengubah kolom tanggal menjadi datetime
df['dteday'] = pd.to_datetime(df['dteday'])

# Membuat Filter untuk menampilkan tren peminjaman sepeda berdasarkan cuaca
weather_filter = st.sidebar.selectbox("Silahkan pilih Kondisi Cuaca:", options=df['weather_name'].unique())
filtered_data = df[df['weather_name'] == weather_filter]

# Fungsi untuk menampilkan tren peminjaman sepeda
def plot_trend(df):
    fig, ax = plt.subplots(figsize=(10, 5))
    sns.lineplot(x=df['dteday'], y=df['cnt'], color='blue')
    ax.set_title(f"Tren Peminjaman Sepeda Berdasarkan Cuaca ({weather_filter})")
    ax.set_xlabel("Tanggal")
    ax.set_ylabel("Jumlah Penyewaan Sepeda")
    plt.xticks(df['dteday'][::14], rotation=45)  # Menampilkan tanggal setiap 7 hari
    st.pyplot(fig)

# Menampilkan visualisasi untuk setiap cuaca
st.subheader(f"Berikut adalah hasil Visualisasi Peminjaman Sepeda untuk Cuaca {weather_filter}")
...
# Membuat tombol untuk memilih analisis yang akan ditampilkan
